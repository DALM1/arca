...
pub fn run(connection: &mut Connection, workspace_root: &Path, once: bool) -> Result<()> {
    initial_scan(connection, workspace_root)?;
    ...
    while running.load(Ordering::SeqCst) {
        match receiver.recv_timeout(Duration::from_millis(500)) {
            Ok(Ok(event)) => handle_event(connection, workspace_root, event)?,
...
fn initial_scan(connection: &mut Connection, workspace_root: &Path) -> Result<()> {
    let records = scanner::scan_workspace(workspace_root)?;
    db::replace_file_index(connection, &records)?;
    println!(
        "Workspace scanne: {} fichiers indexes depuis {}",
        records.len(),
...
fn handle_event(connection: &Connection, workspace_root: &Path, event: Event) -> Result<()> {
